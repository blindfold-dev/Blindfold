...
export function createLocalRequest(locales?: string[]) {
  ...
  return async function localRequest<T = unknown>(
    endpoint: string,
    ...
    if (endpoint === '/discover') {
      throw new Error(
        'Discover is not available in local mode. Set an API key for cloud-based discovery.'
      );
    }
    ...
    if (texts && texts.length > 0) {
      ...
      return { results, total: texts.length, succeeded, failed } as unknown as T;
    }

    if (!text) throw new Error('text is required');

    const result = await (client[method] as Function).call(client, text, config);
